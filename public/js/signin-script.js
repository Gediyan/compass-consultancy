...
// Add this to signin-script.js (replace with your config)
const firebaseConfig = {
apiKey: "AIzaSyAaMiRIrB8YzF0-nRLfpM6DcMbB_IuyNdQ",
authDomain: "compass-aeped.firebaseapp.com",
projectId: "compass-aeped",
storageBucket: "Compass AEPED.appspot.com",
    ...
    if (isValid) {
        try {
            ...
            await setDoc(doc(db, "users", user.uid), {
                name: name,
                email: email,
                accountType: accountType, // 'customer', 'staff', or 'admin'
                createdAt: new Date().toISOString()
            });
...
document.addEventListener('DOMContentLoaded', socialButtonLogin);

function socialButtonLogin() {
    ...
    socialButtons.forEach(button => {
        button.addEventListener('click', async function() {
            console.log("Social login script loaded", button.classList);
            try {
            let provider;
